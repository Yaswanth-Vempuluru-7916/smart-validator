...
const TextArea: React.FC<TextAreaProps> = ({ label, error, validate, ...props }) => {
  ...
  return (
    <div className="mb-4 group">
      <label 
        className="block text-sm font-medium mb-2 text-gray-300 transition-colors group-hover:text-gray-200" 
        htmlFor={props.id}
      >
        {label}
      </label>
      <div className="relative">
        <textarea
          {...props}
          className={`w-full px-4 py-3 bg-gray-800/50 backdrop-blur-sm border-2 rounded-lg 
